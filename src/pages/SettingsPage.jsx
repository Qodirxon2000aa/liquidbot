...
export const SettingsPage = () => {
  ...
  const [isNight, setIsNight] = useState(document.body.classList.contains('night-mode'));
  const [textSize, setTextSize] = useState('medium');

  useEffect(() => {
    // Sync with initial state
    const currentSize = ['small', 'medium', 'large'].find(size => 
      document.body.classList.contains(`text-${size}`)
    ) || 'medium';
  ...
  const changeTextSize = (size) => {
    ['small', 'medium', 'large'].forEach(s => document.body.classList.remove(`text-${s}`));
          ...
          <div className="flex items-center justify-between p-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-[10px] text-zinc-500">Switch between light and dark themes</p>
              </div>
            </div>
            <button 
              onClick={toggleDark}
          ...
          <div className="flex items-center justify-between p-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-[10px] text-zinc-500">Extra dark theme for night usage</p>
              </div>
            </div>
            <button 
              onClick={toggleNight}
            ...
            >
              <div className={`absolute top-1 w-4 h-4 bg-white rounded-full transition-all ${isNight ? 'left-7' : 'left-1'}`} />
            </button>
          </div>
        </Card>
      </div>
